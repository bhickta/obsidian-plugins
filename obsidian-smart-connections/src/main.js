...
import { create_smart_connections_api } from "./utils/smart_connections_api.js";
import {
  load_embedding_cache_sync_settings,
  maybe_auto_pull_embedding_cache,
  pull_embedding_cache,
  push_embedding_cache,
  restore_latest_embedding_cache_backup,
} from "./utils/embedding_cache_sync.js";
...
export default class SmartConnectionsPlugin extends SmartPlugin {
  ...
  async onload() {
    await load_embedding_cache_sync_settings(this);
    await maybe_auto_pull_embedding_cache(this);
    this._api = create_smart_connections_api(this);
  ...
  onunload() {
    console.debug("Unloading Smart Connections plugin");
    this.notices?.unload();
  ...
  get commands() {
    return {
      ...
      embedding_cache_sync_push: {
        id: 'embedding-cache-sync-push',
        name: 'Embedding Cache Sync: Push cache',
        callback: async () => {
          await push_embedding_cache(this);
        }
      },
      embedding_cache_sync_pull: {
        id: 'embedding-cache-sync-pull',
        name: 'Embedding Cache Sync: Pull cache',
        callback: async () => {
          await pull_embedding_cache(this);
        }
      },
      embedding_cache_sync_restore_backup: {
        id: 'embedding-cache-sync-restore-backup',
        name: 'Embedding Cache Sync: Restore latest backup',
        callback: async () => {
          await restore_latest_embedding_cache_backup(this);
        }
      },
    };
  }
